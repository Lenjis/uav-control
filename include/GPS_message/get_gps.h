...
typedef struct{
	...
	float roll_acc,pitch_acc,yaw_acc;//(rad)1考 angles accuracy	 
	...
	float veln_acc,vele_acc,veld_acc;//(m/s)1考 velocity in directions accuracy
	...
	float lat_acc,lon_acc,alt_acc;//(m)1考 position accuracy
	...
	float course_acc;//(deg)1考 course accuracy
	...
	float gps_hdt_acc;//(deg)1考 GPS true heading acuracy
	
	float gps_pitch;//(deg)pitch angle from the master to the rover
	
	float gps_pitch_acc;//(deg)1考 pitch estimated accuracy
