...
/*
	By     : Yan Gongmin @ NWPU
	Date   : 2015-02-17, 2017-07-19, 2018-02-06
...
public:
	...
	CVect3 operator-()const;				// minus
	double norm(const CVect3 &v);					// vector norm
	double normXY(const CVect3 &v);					// vector norm or X & Y components
	CVect3 sqrt(const CVect3 &v);					// sqrt
	CVect3 pow(const CVect3 &v, int k=2);			// power
	...
	//friend CVect3 q2att(const CQuat &qnb);
...
public:
  ...
  //CQuat& operator(const CQuat &q);
	CQuat operator~() const;		// quaternion conjugate
...
public:
	...
	CMat3 operator-()const;					// minus
	//CMat3 operator~() const;					// matirx transposition
	//CMat3 operator*(double f, const CMat3 &m);		// scale multiply matirx
	double det(const CMat3 &m);						// matirx determination
	CMat3 inv(const CMat3 &m);						// matirx inverse
	CVect3 diag(const CMat3 &m);						// diagonal of a matrix
	CMat3 diag(const CVect3 &v);						// diagonal matrix
	CMat3 dv2att(CVect3 &va1, const CVect3 &va2, CVect3 &vb1, const CVect3 &vb2);  // attitude determination using double-vector
...
public:
	...
	//CVect operator~() const;		// vector transposition
...
public:
	...
	CMat array2mat(const double *f, int r, int c);	// convert array to mat
	//CMat operator~() const;				// matirx transposition
	void symmetry(CMat &m);						// matirx symmetrization
	double norm1(CMat &m);						// 1-norm
...
extern CVect3           _delaygy_element_1;        // buffered sample -1
extern CVect3           _delaygy_element_2;        // buffered sample -2
extern CVect3           _delayac_element_1;        // buffered sample -1
extern CVect3           _delayac_element_2;        // buffered sample -2
class  LowPassFilter2p
{
public:
    // constructor
    LowPassFilter2p(void);
    ...
    CVect3 applygyro(const CVect3 &gyro);
		CVect3 applyaccel(const CVect3 &accel);
    /**
    ...
    float get_cutoff_freq(void) const {
		...
		CVect3 resetgyro(const CVect3 &gyro);
		CVect3 resetaccel(const CVect3 &accel);
...
public:
  ...
  CVect3 wib, fb, fn, an, web, wnb, att, vn, vb, pos, eb, db, _tauGyro, _tauAcc, leverarm;
...
public:
	...
	double kftk;
	int nq, nr, measflag;
	CMat Ft, Pk, Hk, Qtest, Qmid, Qset;
	CVect Xk, Zk, Zm, Qt, Rt, rts, Xmax, Pmax, Pmin,
		Rmax, Rmin, Rbeta, Rb,				// measurement noise R adaptive
		FBTau, FBMax, FBXk, FBTotal;		// feedback control

	void Init(void);				// initialize Qk,Rk,P0...
	void SetFt(void);				// process matrix setting
	void SetHk(void);				// measurement matrix setting
	//void SetMeas(void);				// set measurement
	void Feedback(double fbts);			// feed back
...
{
public:
	double tdts, Pz0, meantdts;
	int iter, ifn, measRes, correct;
	CMat Fk, Pk1; 
	CVect Pxz, Qk, Kk, Hi, tmeas;
	CVect3 meanfn;

	CSINSTDKF(int nq0, int nr0);
  //virtual void Measup(void);
	void TDUpdate(const CVect3 *wm, const CVect3 *vm, int nSamples, double tk, int nStep=1);  // Time-Distributed Update
};
...
public:
	...
	CMat3 Cnb;
	CQEAHRS();
	void Init(const CQuat &qua0, double ts);
	void SetMeas(const CQuat& q, const CVect3& acc, const CVect3& mag, double ts);
	void Update(const CVect3 &gyro, const CVect3 &acc, const CVect3 &mag, double ts);
};

class DKFQEAHRS :public CKalman
{
public:
	CSINS sins;
	CMat3 Cnb;
	...
	CVect3 vel_last;
	DKFQEAHRS();
	void Init(const CVect3& att, double ts);
	void SetMeas(const CVect3& acc, const CVect3& mag, double ts);
	void Update(const CVect3& gyro, const CVect3& acc, const CVect3& mag, double ts);
	void Feed(double fbts);
};
