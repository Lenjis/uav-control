...
void LAW_Out(void)
{
    ...
    for(i=MPWMOUTCH1;i<=MPWMOUTCH8;i++)  pwm_set(i, MPWMOUT[i].pwmhi * 5);//MpwmOut(i);
}
...
{
    PWMout[0].min= -20.0f;    PWMout[0].max=  20.0f;          /*[ailerion]*/
    PWMout[1].min= -18.5f;    PWMout[1].max=  18.5f;          /*[elevator]*/ 
