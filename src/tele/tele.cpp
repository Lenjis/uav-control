...
void TELE_FrameB(unsigned char Buf[]) {
    ...
    Buf[1] = 0x90;
    Buf[2] = 'B';
    src.DW = (INT32S)(lon_gps * 1e6);   Buf[3] = src.B[0];  Buf[4] = src.B[1];  Buf[5] = src.B[2];  Buf[6] = src.B[3];  /*[经度]*/  
    src.DW = (INT32S)(lat_gps * 1e6);   Buf[7] = src.B[0];  Buf[8] = src.B[1];  Buf[9] = src.B[2];  Buf[10] = src.B[3]; /*[纬度]*/ 
    ...
    src.W[0] = used_gps;                Buf[18] = used_gps; Buf[19] = 0;        Buf[20] = 0;                            /*[可见星数]*/   
    if(!ss_AHRS.fail)
        SetBit(&Buf[20], 4); /*[AHRS数据链通]*/
    else
        ClearBit(&Buf[20], 4);
    if(!ss_Mpxv.fail)
        SetBit(&Buf[20], 5); /*[空速数据链通]*/
    else
        ClearBit(&Buf[20], 5);
    if(!ss_Gps.fail)
        SetBit(&Buf[20], 6); /*[GPS数据链通]*/
    else
        ClearBit(&Buf[20], 6);
    if(tag_DGPS)
        SetBit(&Buf[20], 7); /*[DGPS]*/
    else
        ClearBit(&Buf[20], 7);
    ...
    if(OnStop_eng)
        SetBit(&Buf[21], 2); /*[发动机预停]*/

    if(on_safe)
        SetBit(&Buf[21], 3); /*[安控状态]*/
                             /*[Buf[3].5~7=自主飞行模态]*/

    Buf[22] = 0; /*[系统状态字4]*/
    if(on_sky)
        SetBit(&Buf[22], 0); /*[飞机在空中]*/

    if(tag_opt)
        SetBit(&Buf[22], 2); /*[辅助控制状态]*/
